        ...
        {({ open }) => (
          <>
            {variant === "colored" && selectedMenu?.icon && (
              <span className="flex h-5 w-5 items-center justify-center mr-2 ml-2">
                {getIcon({
                  iconList: groupIcons,
      ...
      >
        <Menu.Items
          ...
          className={cn(
            ...
            {
              "border border-gray-200 right-0  left-0 origin-top-left":
                variant === "minimal",
              "right-0 origin-top-right left-0  xl:right-auto xl:left-0 xl:origin-top-left  ":
                variant !== "minimal",
            }
          )}
        >
                ...
                {({ active }) => (
                  ...
                  <div>
                    <Link
                      href={`/${slug}`}
                      className={cn(
                        "flex w-full items-center space-x-4 px-5 py-2.5 text-sm font-semibold capitalize transition duration-200 hover:text-primary focus:outline-0 focus-visible:outline-0 space-x-reverse",
